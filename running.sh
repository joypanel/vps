...
xtls_xray_status=$(systemctl status xray-mini@vless-direct.service | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
if [[ $oovpn == "active" ]]; then
  status_openvpn="${GREEN}OpenVPN Service Is Running ${NC}(${GREEN} ✔ ${NC})"
else
  status_openvpn="${RED}OpenVPN Service Is Not Running ${NC}( Error )"
fi

# Status Service OHP
if [[ $ohpovpn == "active" ]]; then
  status_OHPovpn="${GREEN}OHP Service Is Running ${NC}(${GREEN} ✔ ${NC})"
else
  status_OHPovpn="${RED}OHP Service Is Not Running ${NC}( Error )"
fi

# Status Service  SSH
if [[ $ssh_service == "running" ]]; then
   status_ssh="${GREEN}SSH/Tunnel Service Is Running ${NC}(${GREEN} ✔ ${NC})"
else
   status_ssh="${RED}SSH/Tunnel Service Is Not Running ${NC}( Error )"
fi

# Status Service  Squid
if [[ $squid_service == "running" ]]; then
   status_squid="${GREEN}Squid Service Is Running ${NC}(${GREEN} ✔ ${NC})"
else
   status_squid="${RED}Squid Service Is Not Running ${NC}( Error )"
fi

# Status Service  VNSTAT
if [[ $vnstat_service == "running" ]]; then
   status_vnstat="${GREEN}Vnstat Service Is Running ${NC}(${GREEN} ✔ ${NC})"
else
   status_vnstat="${RED}Vnstat Service Is Not Running ${NC}( Error )"
fi

# Status Service  Crons
if [[ $cron_service == "running" ]]; then
   status_cron="${GREEN}Crons Service Is Running ${NC}(${GREEN} ✔ ${NC})"
else
   status_cron="${RED}Crons Service Is Not Running ${NC}( Error )"
fi

# Status Service  Fail2ban
if [[ $fail2ban_service == "running" ]]; then
   status_fail2ban="${GREEN}Fail2Ban Service Is Running ${NC}(${GREEN} ✔ ${NC})"
else
   status_fail2ban="${RED}Fail2Ban Service Is Not Running ${NC}( Error )"
fi

# Status Service XTLS XRAY
if [[ $xtls_xray_status == "running" ]]; then
   status_xtls_xray="${GREEN}XRAY DIRECT & SPLICE Service Is Running${NC} (${GREEN} ✔ ${NC})"
else
   status_xtls_xray="${RED}XRAY TCP XTLS Service Is Not Running${NC} ( Error )"
fi

# Status Service TLS XRAY
if [[ $tls_xray_status == "running" ]]; then
   status_tls_xray="${GREEN}XRAY Vmess WS Service Is Running${NC} (${GREEN} ✔ ${NC})"
else
   status_tls_xray="${RED}XRAY Vmess WS Service Is Not Running${NC} ( Error )"
fi

# Status Service Non TLS XRAY
if [[ $nontls_xray_status == "running" ]]; then
   status_nontls_xray="${GREEN}XRAY Vmess WS None TLS Service Is Running (${GREEN} ✔ ${NC})"
else
   status_nontls_xray="${RED}XRAY Vmess WS None TLS Service Is Not Running ${NC}( Error )${NC}"
fi

# Status Service XRAY Vless HTTPS
if [[ $xvless_tls_xray_status == "running" ]]; then
  status_tls_xvless="${GREEN}XRAY Vless WS Service Is Running${NC} (${GREEN} ✔ ${NC})"
else
  status_tls_xvless="${RED}XRAY Vless WS Service Is Not Running ${NC}( Error )${NC}"
fi

# Status Service XRAY Vless HTTP
if [[ $xvless_nontls_xray_status == "running" ]]; then
  status_nontls_xvless="${GREEN}XRAY Vless WS None TLS Service Is Running${NC} (${GREEN} ✔ ${NC})"
else
  status_nontls_xvless="${RED}XRAY Vless WS None TLS Service Is Not Running ${NC}( Error )${NC}"
fi
...
then
  status_ssr="${GREEN}ShadowsocksR Service Is Running (${GREEN} ✔ ${NC})"
else
  status_ssr="${RED}ShadowsocksR Service Is Not Running ${NC}( Error )${NC}"
fi


# Sodosok
if [[ $status_text == "active" ]] ; then
  status_sodosok="${GREEN}Shadowsock Service Is Running${NC} (${GREEN} ✔ ${NC})${NC}"
else
  status_sodosok="${RED}Shadowsocks Service Is Not Running ${NC}( Error )${NC}"
fi
...
if [ "${status_text}" == "active" ]
then
   status_virus_trojan="${GREEN}Trojan Service Is Running (${GREEN} ✔ ${NC})"
else
   status_virus_trojan="${RED}Trojan Service Is Not Running ${NC}( Error )${NC}"
fi
# Status Service Wireguard
if [[ $swg == "active" ]]; then
  status_wg="${GREEN}Wireguard Service Is Running (${GREEN} ✔ ${NC})"
else
  status_wg="${RED}Wireguard Service Is Not Running ${NC}( Error )${NC}"
fi

# Status Service Dropbear
if [[ $dropbear_status == "running" ]]; then
   status_beruangjatuh="${GREEN}Dropbear Service Is Running${NC} (${GREEN} ✔ ${NC})"
else
   status_beruangjatuh="${RED}Dropbear Service Is Not Running ${NC}( Error )${NC}"
fi

# Status Service Stunnel
if [[ $stunnel_service == "running" ]]; then
   status_stunnel="${GREEN}Stunnel Service Is Running ${NC}(${GREEN} ✔ ${NC})"
else
   status_stunnel="${RED}Stunnel Service Is Not Running ${NC}( Error )}"
fi
...
echo -e "   XRAY Vmess WS TLS : $status_tls_xray"
echo -e "   XRAY Vmess WS None TLS: $status_nontls_xray"
echo -e "   XRAY Vless TLS    : $status_tls_xvless"
echo -e "   XRAY Vless WS None TLS: $status_nontls_xvless"
